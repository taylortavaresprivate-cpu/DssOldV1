...
function cfg.loadConfig()
	...
	cfg.FFB_DAMPER           = getf("ffb_damper",             cfg.FFB_DAMPER)
	cfg.FFB_LATERAL          = getf("ffb_lateral",            cfg.FFB_LATERAL)
	cfg.FFB_GAMMA            = getf("ffb_gamma",              cfg.FFB_GAMMA)
	cfg.STEER_MAX_RATE       = getf("steer_max_rate",         cfg.STEER_MAX_RATE)
	if cfg.FFB_GAMMA         < 0.5  then cfg.FFB_GAMMA         = 0.5  end
	if cfg.FFB_GAMMA         > 2.5  then cfg.FFB_GAMMA         = 2.5  end
	if cfg.FFB_DAMPER        < 0.0  then cfg.FFB_DAMPER        = 0.0  end
	if cfg.FFB_DAMPER        > 3.0  then cfg.FFB_DAMPER        = 3.0  end
	if cfg.FFB_LATERAL       < 0.0  then cfg.FFB_LATERAL       = 0.0  end
	if cfg.FFB_LATERAL       > 2.0  then cfg.FFB_LATERAL       = 2.0  end
	if cfg.STEER_MAX_RATE    < 0.5  then cfg.STEER_MAX_RATE    = 0.5  end
	if cfg.STEER_MAX_RATE    > 10.0 then cfg.STEER_MAX_RATE    = 10.0 end
